...
if __name__ == '__main__':#Se dice a python que este es el programa principal
    ...
    contador_imagenes = 0 #Contador para saber la cantidad de imagenes que se evaluaran
    ...
    for doc in filenames: #Se inica un ciclo para leer los archivos en la carpeta raiz
        if doc == 'Manual_landmarks.csv': #Si se encuentra el archivo con las landmarks manuales
            resultados(Directory + '/Alignded_Images', contador_imagenes, imagen_cuadrada.shape[0],imagen_cuadrada.shape[1], 
		       Directory + '/Manual_landmarks.csv', Directory + '/Alignded_Images/Landmarks.csv') #Se genera el archivo log
